...
export default function ChatItem({
  ...
  const isImage = fileType && !isPDF;
  const router = useRouter();
  const params = useParams();

  // Reset form & display the current message in edit field
  useEffect(() => {
    form.reset({
      content: content,
    });
        ...
        <div className="flex flex-col w-full">
          ...
          {isImage && (
            <a
              ...
              className="relative aspect-square rounded-sm mt-2 overflow-hidden border flex items-center
          ...
          {/* Normal message */}
          {/* fileUrl && !edited && !deleted => Normal message */}
          {!fileUrl && !isEditing && (
            <p
              className={cn(
